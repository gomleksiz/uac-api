...
class Workflows:
    ...
    def add_child_vertex(
        ...
        if parent_vertex_id:
            ...
            response = self.get_vertices(
                workflowname=workflow_name, vertex_id=parent_vertex_id
            )
        else:
            response = self.get_vertices(
                workflowname=workflow_name, taskname=parent_task_name
            )
            parent_vertex_id = response[0]["vertexId"]

        if vertex_id:
            new_vertex_id = str(vertex_id)
            response = self.add_vertex(
                workflowname=workflow_name, task=task_name, vertex_id=new_vertex_id
            )
        else:
            response = self.add_vertex(workflowname=workflow_name, task=task_name)
            new_vertex_id = response["vertexId"]

        response = self.add_edge(
            workflowname=workflow_name,
            sourceid=parent_vertex_id,
            targetid=new_vertex_id,
        )
        if auto_arrange:
    ...
    def auto_arrange_vertices(self, workflow_name=None, payload=None):
        ...
        positions = WorkflowsVertexPositions(
            workflow.get("workflowEdges", [])
        ...
        for vertex in vertices:
            ...
            if current_x != x or current_y != y:
                self.log.debug(
                    f"VertexId {id} X: {vertex['vertexX']}=>{x} Y: {vertex['vertexY']}=>{y}"
                )
